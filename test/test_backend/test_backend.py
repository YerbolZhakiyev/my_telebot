...
@app.route('/test_orders', methods = ['GET', 'POST'])
def orders(): 
    if request.method == 'GET':
        ...
        results = []
        for row in rows:
            results.append({'id': row[0], 'description': row[1], 'from_address': row[2], 'to_address': row[3], 'units': row[4], 'weight': row[5], 'phone': row[6], 'tg_id': [7]})
        orders = json.dumps({
            'data': results
            }, ensure_ascii = False)
...
@app.route('/test_customers', methods = ['GET', 'POST'])
def customers(): 
    if request.method == 'GET':
